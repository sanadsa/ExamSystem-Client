...
export class QuestionFormComponent implements OnInit {
  ...
  private initQuesForm() {
    return this.fb.group({
      question: this.fb.group({
        questionType: [
          this.question.QuestionType || "SingleChoice",
          Validators.required,
        ],
  ...
  private editQuestion(questionToEdit) {
    this.questionService.editQuestion(questionToEdit).subscribe(
      (res) => {
  ...
  private createAnswers(questionId) {
    ...
    for (let ansControl of this.answersFormArray["controls"]) {
      let answer: Answer = {
        ...
        CorrectAnswer:
          ansControl.value.IsCorrect === undefined
            ? true
            : ansControl.value.IsCorrect,
        Info: ansControl.value.Info,
        ID: ansControl.value.ID,
      };

      if (answer.ID != 0) {
        this.questionService.updateAnswer(answer).subscribe(
          (response) => {},
  ...
  public navToQuestionsList() {
    this.router.navigate([
      this.constantFields.questionsListRoute,
      { field: this.field },
    ]);
  }
